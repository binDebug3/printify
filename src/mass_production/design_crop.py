...
def _find_foreground_bounding_box(
    ...
    for y_coord in range(image.height):
        for x_coord in range(image.width):
            ...
            if _is_foreground_pixel(pixel, background_color, tolerance):
...
def _detect_background_color(image: Image.Image) -> tuple[int, int, int, int]:
    ...
    for x_coord in range(image.width):
        border_pixels.append(pixels[x_coord, 0])
        border_pixels.append(pixels[x_coord, image.height - 1])
    for y_coord in range(image.height):
        border_pixels.append(pixels[0, y_coord])
        border_pixels.append(pixels[image.width - 1, y_coord])
