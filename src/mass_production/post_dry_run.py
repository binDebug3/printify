    ...
    if products_dir.exists():
        for keyword_dir in sorted(
            ...
            if not keyword_dir.is_dir():
                continue
            nested_candidate = keyword_dir / folder_slug
            if nested_candidate.exists() and nested_candidate.is_dir():
                return nested_candidate

    existing = (
        [child.name for child in products_dir.glob("*/*") if child.is_dir()]
        if products_dir.exists()
        else []
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "folder_slug",
        help=(
            "Folder slug under data/products/<keyword>/ generated by dry run "
            "(slug only)"
        ),
    )
...
def main() -> None:
    ...
    if product_id:
        try:
            ...
            log_action(f"Schedule update failed for product '{product_id}': {exc}")
        try:
