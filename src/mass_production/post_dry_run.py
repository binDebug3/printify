...
def _resolve_folder(folder_slug: str) -> Path:
    ...
    Args:
        folder_slug: Folder name under data/images.
    ...
    candidate = images_dir / folder_slug
    if candidate.exists() and candidate.is_dir():
        return candidate

    existing = (
        [child.name for child in images_dir.iterdir() if child.is_dir()]
        if images_dir.exists()
        else []
    )
    suggestion = _closest_folder_name(folder_slug, existing)
    if suggestion is None:
        _abort(f"Folder '{folder_slug}' not found under {images_dir}.")
    _abort(
        f"Folder '{folder_slug}' not found under {images_dir}. "
        f"Did you mean '{suggestion}'?"
    )
    return candidate
...
def _find_color_mockup_cropped(folder: Path) -> Path:
    ...
    candidates = [
        child for child in folder.iterdir() if child.is_file() and pattern.match(child.name)
    ]
    if not candidates:
...
def _load_selected_colors(idea_json_path: Path, color_to_ids: Dict[str, List[int]]) -> List[str]:
    ...
    """
    log_action(f"Loading selected colors against variant_map colors from '{idea_json_path}'")
    ...
    log_action("No valid shirt_colors in ideas.json; falling back to all variant_map colors")
    return fallback
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Create one Printify draft from a dry-run folder")
    parser.add_argument(
        "folder_slug",
        help="Folder name under data/images generated by dry run (slug only)",
    )
...
def main() -> None:
    ...
    transparent_path = _require_file(folder / "design_transparent.png", "design_transparent.png")
    ...
    base_price = client.pick_base_price_usd(constants.BASE_PRICE_USD, constants.PRICE_STDEV_USD)
    ...
    mockup_image_id = (
        mockup_upload_result.get("id") if isinstance(mockup_upload_result, dict) else None
    )
    mockup_src = (
        mockup_upload_result.get("preview_url") if isinstance(mockup_upload_result, dict) else None
    )
    if not mockup_image_id:
        _abort(
            "Mockup upload succeeded but returned no image id, so default image cannot be set."
        )
    log_action(f"Using uploaded mockup image id for default image attempt: '{mockup_image_id}'")
    ...
    if product_id:
        try:
            ...
            log_action(
                f"Default mockup update attempt failed (continuing): {exc}"
            )
