...
class TestPublishProduct:
    ...
    def test_loads_shop_id_when_not_provided(self):
        ...
        mock_resp = MagicMock(status_code=200)
        with patch("tools.requests.post", return_value=mock_resp), patch(
            "tools.load_shop_id", return_value=SHOP_ID
        ) as mock_load:
    ...
    def test_loads_token_when_not_provided(self):
        ...
        mock_resp = MagicMock(status_code=200)
        with patch("tools.requests.post", return_value=mock_resp), patch(
            "tools.load_api_token", return_value=TOKEN
        ) as mock_load:
...
class TestGetAllProducts:
    ...
    def test_re_raises_read_timeout(self):
        ...
        with patch("tools.requests.get", side_effect=requests.exceptions.ReadTimeout):
    ...
    def test_loads_shop_id_when_not_provided(self):
        """Calls load_shop_id when shop_id is None."""
        mock_resp = MagicMock(status_code=200)
        with patch("tools.requests.get", return_value=mock_resp), patch(
            "tools.load_shop_id", return_value=SHOP_ID
        ) as mock_load:
...
class TestGetAllProductIds:
    ...
    def test_returns_title_id_tuples(self, tmp_path):
        ...
        with patch("tools.get_all_products", return_value=mock_resp):
